...
export default function NInput({ label, val, set, pre, suf, step = "0.1", min = "0", ro = false }) {
  const [foc, setFoc] = useState(false);
  return (
    <div className="min-w-0">
      <label className="block text-xs font-medium uppercase tracking-wide mb-1 truncate" style={{ color: T.low }}>
        {label}
      </label>
      <div
      ...
      >
        ...
        <input
          ...
          onBlur={() => setFoc(false)}
          className="flex-1 px-2 py-1.5 text-sm outline-none bg-transparent min-w-0 w-0 font-medium"
          style={{ color: T.high }}
        />
        {suf && (
